...
| Shared labels / API path | `Constants.pm` English msgids; `I18N.pm` applies `po/*.po` at runtime. JS copies English fallbacks in `js/spc-config.js`. | Koha 24.11 does not install plugin PO files (Bug 37472). Keep Constants and `spc-config.js` in sync. |
