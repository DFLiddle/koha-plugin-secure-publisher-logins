...
| Shared labels / API path | `Constants.pm` English msgids; `I18N.pm` applies `po/*.po` at runtime (see [I18N_LOCALES.md](I18N_LOCALES.md)). Perl hooks inject translated labels into `window.SPC`; JS fallbacks in `js/spc-config.js`. | Koha 24.11 does not install plugin PO files (Bug 37472). Keep Constants and `spc-config.js` in sync. |
